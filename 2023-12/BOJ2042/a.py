...
# 트리의 구간 left~right
# 현재 노드 x
def find_tree(start, end, x, left, right):
    # 구하고 싶은 구간(start~end)가 현재 트리 구간에 포함 X
    if end < left or right < start:
        return 0
    # 구하고 싶은 구간(start~end) 안에 현재 트리 포함
    if start <= left and right <= end:
        return seg_tree[x]
    # 구간이 겹치는 경우
    mid = (left + right) // 2
    left_value = find_tree(start, end, x * 2, left, mid)
    right_value = find_tree(start, end, x * 2 + 1, mid + 1, right)
...
for _ in range(m + k):
    ...
    # b번째 수를 c로 바꾸기
    if a == 1:
        update_tree(1, 0, n - 1, start - 1, end)
    # b번째 수부터 c번째 수까지 합 구하기
    else:
        s = find_tree(start - 1, end - 1, 1, 0, n - 1)
        print(s)
